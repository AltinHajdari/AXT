...
TEST_PROMPT_FILE = "resources/prompts/generate_code_system.txt"
...
def test_load_prompt_template_from_packaged_resource(monkeypatch: pytest.MonkeyPatch) -> None:
    service = CodegenPromptService(config=AppConfig(draft_mapping_prompt_file=TEST_PROMPT_FILE))
    fake_resource = _FakeResourcePath(text="SYSTEM TEMPLATE")

    monkeypatch.setattr(
        "girbridge.services.codegen_prompt_service.resources.files",
...
def test_build_prompt_contains_required_sections() -> None:
    ...
    prompt = service._build_prompt(
        ...
        attached_files=["mapping.yaml", "a.xsd"],
    )
    ...
    assert "- mapping.yaml" in prompt
    assert "- a.xsd" in prompt
    assert "## REQUIRED INPUT FILES" in prompt
    assert "- Mapping file: mapping.yaml" in prompt


def test_generate_code_prompt_writes_output_and_attachments(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    service = CodegenPromptService(config=AppConfig(draft_mapping_prompt_file=TEST_PROMPT_FILE))
    ...
    assert "- mapping.yaml" in written
    assert "- sample.xml" in written
    assert "- a.xsd" in written
    assert "- b.xsd" in written
